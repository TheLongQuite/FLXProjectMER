...
public static class StructExtensions
{
    ...
    public static Vector2 ToVector2(this object jObject)
    {
        ...
        return new(Convert.ToSingle(dict["x"]), Convert.ToSingle(dict["y"]));
    }

    public static bool TryParseToFloat(this string s, out float result) => float.TryParse(s.Replace(',', '.'),
    ...
    public static bool TryGetVector(string x, string y, string z, out Vector3 vector)
    {
        ...
        if (!x.TryParseToFloat(out float xValue) || !y.TryParseToFloat(out float yValue) ||
            !z.TryParseToFloat(out float zValue))
            return false;
