...
public class Modify : ICommand
{
    ...
    public bool Execute(ArraySegment<string> arguments, ICommandSender sender, out string response)
    {
        ...
        if (typeof(ICollection).IsAssignableFrom(foundProperty.PropertyType))
            ...
            result = HandleString(out response);
        ...
        bool HandleCollection(out string response)
        {
            ...
            Type listType =
                foundProperty.PropertyType.GetInterfaces().First(x => x.IsGenericType).GetGenericArguments()[0];
            ...
            switch (arguments.At(1).ToLower())
            {
                ...
                case "add":
                {
                    for (int i = 2; i < arguments.Count; i++)
                    {
                        try
                        {
                            object value = TypeDescriptor.GetConverter(listType)
                                .ConvertFromInvariantString(arguments.At(i));

                            foundProperty.PropertyType.GetMethod("Add").Invoke(listInstance, [value]);
                        }
                        catch (Exception)
                        {
                            response = $"\"{arguments.At(i)}\" is not a valid argument! The value should be a {listType
                            } type.";
        ...
        bool HandleNonString(out string response)
        {
            ...
            try
            {
                object value = TypeDescriptor.GetConverter(foundProperty.PropertyType)
                    .ConvertFromInvariantString(arguments.At(1));
            ...
            catch (Exception)
            {
                ...
                {
                    sb.Append($"\"{arguments.At(1)}\" is not a valid argument! The value should be a {
                        foundProperty.PropertyType} type.");
                }

                if (foundProperty.PropertyType.IsEnum)
                {
                    sb.AppendLine();
                    sb.Append($"{foundProperty.PropertyType.ToString().Split('.').Last()
                    } values (use either text name or number, sum numbers for multiple flags)");
        ...
        bool HandleString(out string response)
        {
            ...
            try
            {
                foundProperty.SetValue(instance,
                    TypeDescriptor.GetConverter(foundProperty.PropertyType)
                        .ConvertFromInvariantString(StringBuilderPool.Shared.ToStringReturn(spacedStringBuilder)));
            }
            catch (Exception)
            {
                response = $"\"{arguments.At(1)}\" is not a valid argument! The value should be a {
                    foundProperty.PropertyType} type.";
